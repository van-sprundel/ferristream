...
// TorrentIndexer trait adapter for Prowlarr
use super::{IndexerError, TorrentIndexer};
...
impl ProwlarrIndexer {
    pub fn new(config: &ProwlarrConfig) -> Self {
        Self {
            client: ProwlarrClient::new(config),
...
#[async_trait]
impl TorrentIndexer for ProwlarrIndexer {
    fn name(&self) -> &str {
        "Prowlarr"
    }

    fn is_enabled(&self) -> bool {
        true // Prowlarr is always enabled if configured
    }
    ...
    async fn search(
        ...
        // Get all usable indexers from Prowlarr
        let indexers = self
            .client
            .get_usable_indexers()
            .await
            .map_err(|e| match e {
                ProwlarrError::RequestError(req_err) => IndexerError::RequestError(req_err),
                ProwlarrError::InvalidResponse(msg) => {
                    IndexerError::ParseError(format!("Prowlarr: {}", msg))
                }
            })?;
        ...
        tracing::debug!(count = indexers.len(), "prowlarr: querying indexers");

        // Search all indexers in parallel (limit concurrency to 5)
        use futures::stream::{self, StreamExt};

        let results: Vec<TorrentResult> = stream::iter(indexers)
            .map(|indexer| {
                let torznab = &self.torznab;
            ...
            .buffer_unordered(5) // Max 5 concurrent searches
            .collect::<Vec<_>>()
            .await
            .into_iter()
            .flatten()
